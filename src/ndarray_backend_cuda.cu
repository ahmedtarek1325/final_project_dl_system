...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid< size ){
    size_t size_rem = gid ;
    size_t index,block_order;
    index= 0 ;
    for(int i = shape.size-1;i>=0;i--){
      
      block_order = size_rem%shape.data[i];
      index+= block_order*strides.data[i];
      size_rem /= shape.data[i];
    }

    out[gid] = a[index+offset];
  }
  /// END YOUR SOLUTION
}
...
__global__ void EwiseSetItemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                              CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the Ewise set item opeation.  This should effectively set a single entry in 
   * non-compact output out, to the corresponding item (at location gid) in the compact array a.
   * 
   * Args:
   *   a: CUDA pointer to a array
   *   out: CUDA point to out array
...
__global__ void ScalarSetItemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape,
  ...
  /**
   * The CUDA kernel for the Ewise set item opeation.  This should effectively set a single entry in 
   * non-compact output out, to constant val.
   * 
   * Args:
   *   a: CUDA pointer to a array
   *   out: CUDA point to out array
  ...
  /// BEGIN YOUR SOLUTION
  if (gid< size ){
    uint32_t size_rem = gid ;
    uint32_t index,block_order;
    index= 0 ;
    for(int i = shape.size-1;i>=0;i--){
...
__global__ void EwiseLogKernel(const scalar_t* a, scalar_t* out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[gid] =__logf(a[gid]);
}
...
__global__ void EwiseExpKernel(const scalar_t* a, scalar_t* out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[gid] =  __expf(a[gid]);
}
...
// Elementwise and scalar operations
////////////////////////////////////////////////////////////////////////////////
__global__ void MatMulKernel(const scalar_t* a,const scalar_t* b, scalar_t* out, size_t size,int32_t M, int32_t N,int32_t P) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size){
    int32_t i = gid % P; 
    int32_t j = gid / P; 
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size,size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size){
    uint32_t startRP = gid*reduce_size;
    scalar_t max = a[startRP]; 
  
    for (uint32_t i= startRP ;i < startRP+reduce_size ; i++){
      if(a[i]> max) max= a[i]; 
    }
    out[gid] = max ; 
  }
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t size,size_t reduce_size) {
  ...
  if (gid < size){
    ...
    for (uint32_t i= startRP ;i < startRP+reduce_size ; i++) sum+= a[i]; 
